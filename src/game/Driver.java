...
import object.Shielder;
import object.Platform;
import object.Player;
import object.SilverAsh;
import object.Sprite;
import object.Vampire;
...
public class Driver extends JPanel implements ActionListener, KeyListener, MouseListener, MouseMotionListener {
	...
	public void update() {
		if (screen.equals("character selection")) {
			if (selecting.equals("SilverAsh")) {
				if (players.size() == 0) {
					players.add(new SilverAsh(400, 450));
				} else {
					players.add(new SilverAsh(950, 450));
				}
				delay(300);
				selecting = " ";
			}
			if (selecting.equals("Vampire")) {
				if (players.size() == 0) {
					players.add(new Vampire(400, 450));
				} else {
					players.add(new Vampire(950, 450));
				}
				delay(300);
				selecting = " ";
			}
			if (selecting.equals("Shielder")) {
				if (players.size() == 0) {
					players.add(new Shielder(400, 450));
				} else {
					players.add(new Shielder(950, 450));
				}
				delay(300);
				selecting = " ";
			}
	...
	public void paint(Graphics g) {
		...
		if (players.size() == numberPlayers) {
			// stage selection currently doesn't work, so we skip it
	...
	// update subsections
	// all the updating that is needed during the fight partattack
	public void fightUpdate() {
		delay(50);
		for (Player player1 : players) {
			boolean footing = false;
			for (Platform platform : platforms) {
				if (intersecting(player1, platform)) {
					footing = true;
					player1.setY(platform.getY() - platform.getHeight() - 30);
					player1.setJumps(player1.getTotalJumps());
				}
			}
			player1.move(footing);

			if (player1.isSpecialAttack() && player1.getCooldown() <= 0 && player1.getResponsive() <= 0) {
				attacks.add(player1.specialAttack());
			} else if (player1.isNormalAttack() && player1.getCooldown() <= 0 && player1.getResponsive() <= 0) {
				attacks.add(player1.regularAttack());
			}

			for (Attack attack : attacks) {
				attack.move();
				if (attack.getDuration() > 0) {
					// checking if attack b should be affecting player1
					if (touching(player1, attack) && attack.getAttacker() != player1
							&& !attack.hitList().contains(player1)
							&& player1.getInvincible() <= 0) {
						// case handling for attacks with special effects
						if (player1 instanceof Shielder && player1.defenseMode()
								&& player1.getDirection() != attack.getDirection()) {
							player1.takeDamage(Math.min(1, attack.getDamage()), attack.getKnockback() / 2,
									attack.getStun(),
									attack.getDirection().equals("right"));
						} else if (attack instanceof NoirShield) {
							player1.takeDamage(attack.getDamage(), attack.getKnockback(), attack.getStun(),
									attack.getX() < player1.getX());
						} else {
							player1.takeDamage(attack.getDamage(), attack.getKnockback(), attack.getStun(),
									attack.getDirection().equals("right"));
							if (attack.getAttacker() instanceof Vampire) {
								attack.getAttacker().restoreHealth(1);
							}
						}
						attack.hit(player1);
					}
	...
	public void characterSelectionPaint(Graphics2D g2) {
		...
		Button selectVampire = new Button(300, 150, 150, 150, "../Vampire/Vampire Portrait.png");
		selected(selectVampire);
		selectVampire.paint(g2);
		if (selectVampire.selected(mouseX, mouseY) && pressed) {
			selecting = "Vampire";
		}

		Button selectShielder = new Button(500, 150, 150, 150, "../Shielder/Shielder Portrait.png");
		selected(selectShielder);
		selectShielder.paint(g2);
		if (selectShielder.selected(mouseX, mouseY) && pressed) {
			selecting = "Shielder";
		}

		// work in progress
		/*
		 * Button selectFrostnova = new Button(700, 150, 150, 150,
		 * "../Frostnova/Frostnova Portrait.png");
	...
	public double[] scale() {
		...
		for (Player player : players) {
			if (player.getX() + player.getWidth() / 2 < smallestX && player.getLives() > 0)
				smallestX = Math.max(player.getX() + player.getWidth() / 2, 0);
			if (player.getY() + player.getHeight() / 2 < smallestY && player.getLives() > 0)
				smallestY = player.getY() + player.getHeight() / 2;
			if (player.getX() + player.getWidth() / 2 > largestX && player.getLives() > 0)
				largestX = Math.min(player.getX() + player.getWidth() / 2, 1500);
			if (player.getY() + player.getHeight() / 2 > largestY && player.getLives() > 0)
				largestY = Math.min(player.getY() + player.getHeight() / 2, 700);
		}
		result[2] = Math.min(
				Math.min(0.4 + 400.0 / (largestY - smallestY + 300), 0.4 + 600.0 / (largestX - smallestX + 300)), 1.3);
	...
	// contact-checking code, intersecting for with ground, touching for attacks and
	// players
	public boolean intersecting(Sprite a, Sprite b) {
		...
		if (Math.abs(bCenterX - aCenterX) < (b.getWidth()) / 2)
			sides = true;

		if (Math.abs(aDown - bUp) < 30)
			top = true;
		return sides && top;
	}
	...
	public boolean touching(Sprite a, Sprite b) {
		...
		if (Math.abs(bCenterX - aCenterX) < (a.getWidth() + b.getWidth()) / 2)
			sides = true;

		if (aUp > bUp && aUp < bDown)
			top = true;
		else if (bUp > aUp && bUp < aDown)
			top = true;
		else if (bDown < aDown && bDown > aUp)
			top = true;
		else if (aDown < bDown && aDown > bUp)
			top = true;
		else if ((aDown + aUp) / 2 < bDown && (aDown + aUp) / 2 > bUp)
			top = true;
		return sides && top;
	}
